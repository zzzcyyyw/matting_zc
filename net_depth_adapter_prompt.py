...
def model_builder(weight_init, args):
    ...
    # num_channels = 4
    # if num_channels > 3:
    #     image_encoder_sd = image_encoder.state_dict()
    #     con1_weights = image_encoder_sd['conv1.weight']
    #     c_out, c_in, h, w = con1_weights.size()
    #     conv1_mod = torch.zeros(c_out, num_channels, h, w)
    #     torch.nn.init.xavier_normal_(conv1_mod)
    #     conv1_mod[:, :3, :, :] = con1_weights
    #     conv1 = image_encoder.conv1
    #     conv1.in_channels = num_channels
    #     conv1.weight = torch.nn.Parameter(conv1_mod)
    #     image_encoder.conv1 = conv1
    #     image_encoder_sd['conv1.weight'] = conv1_mod
    #     image_encoder.load_state_dict(image_encoder_sd)
    ...
    # depth_adapter_ckpt = torch.load('saved_model/adapter/train_adapter_30.pth')
    # model.load_state_dict(depth_adapter_ckpt, strict=True)
